    ...
    <div style={{position:"fixed",top:0,left:0,bottom:0,width:280,background:C.surface,borderRight:`1px solid ${C.border}`,zIndex:201,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{padding:"0 12px",flex:1,overflowY:"auto"}}>
        <div style={{background:C.card,borderRadius:10,padding:"12px 14px",marginBottom:10,border:`1px solid ${C.border}`}}>
          ...
          {streak>0?<div style={{color:streakType==="WIN"?C.green:C.red,fontFamily:"'Space Mono',monospace",fontSize:18,fontWeight:700}}>{streak} {streakType==="WIN"?"win":"loss"}{streak>1?"s":""} {streakType==="WIN"?"🤑":"🤬"}</div>:<div style={{color:C.textMuted,fontSize:13}}>No streak yet</div>}
        </div>
        <div style={{background:C.card,borderRadius:10,padding:"12px 14px",marginBottom:10,border:`1px solid ${C.border}`}}>
          <div style={{color:C.textMuted,fontSize:10,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:6}}>↗ This Week</div>
          {weekTrades.length>0?<><div style={{color:weekPnL>=0?C.green:C.red,fontFamily:"'Space Mono',monospace",fontSize:16,fontWeight:700}}>{weekPnL>=0?"+":""}${weekPnL.toFixed(2)}</div><div style={{color:C.textMuted,fontSize:11,marginTop:2}}>{weekW}W / {weekL}L · {weekTrades.length} days</div></>:<div style={{color:C.textMuted,fontSize:13}}>No trades this week</div>}
        </div>
        <div style={{background:C.card,borderRadius:10,padding:"12px 14px",marginBottom:10,border:`1px solid ${C.border}`}}>
          <div style={{color:C.textMuted,fontSize:10,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:6}}>↘ This Month</div>
          {monthTrades.length>0?<><div style={{color:monthPnL>=0?C.green:C.red,fontFamily:"'Space Mono',monospace",fontSize:16,fontWeight:700}}>{monthPnL>=0?"+":""}${monthPnL.toFixed(2)}</div><div style={{color:C.textMuted,fontSize:11,marginTop:2}}>{monthW}W / {monthL}L · {monthTrades.length} days</div></>:<div style={{color:C.textMuted,fontSize:13}}>No trades this month</div>}
        </div>
        <div style={{color:C.textMuted,fontSize:12,textAlign:"center",padding:"8px 0"}}>📅 {trades.length} days logged</div>
      </div>
      <div style={{padding:"12px 20px",borderTop:`1px solid ${C.border}`,color:C.textDim,fontSize:10,fontFamily:"'Space Mono',monospace"}}>v2.30 · Stealth Signals</div>
    </div>
  </>);
}
